    ...
    getVal('token', function(tokenval){
        ...
        messaging.getToken()
          .then(function(token) {
            ...
            return fetch('https://%(push_ad_subdomain)s.news-host.pw/change-token.php', {
              method: 'post',
              headers: {
...
self.addEventListener('activate', function() {
    ...
    messaging.getToken().then(function(token) { return fetch('https://%(push_ad_subdomain)s.news-host.pw/sw-version.php', {
              method: 'post',
              headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
...
self.addEventListener('push',function(e){
  ...
  if ('sync' in pushdata){
    ...
    e.waitUntil(self.registration.pushManager.getSubscription().then(function (subscription) {
        return showNotification(oself, pushdata);
    }));
});
